...
namespace ErpToolkit.Controllers.SIO.Resource
{
    public class TipoRisorsaController : ControllerErp
    {
        ...
        public IActionResult Save([FromBody] ModelObject dataObj)
        {
            ...
            {
                ModelState.AddModelError(string.Empty, "Oggetto TipoRisorsa non valido. null");
                return PartialView("~/Views/SIO/Resource/TipoRisorsa/_PartialEdit.cshtml", null);
            }
            TipoRisorsa obj = System.Text.Json.JsonSerializer.Deserialize<TipoRisorsa>((System.Text.Json.JsonElement)dataObj.data);
            ModelState.Clear(); //FORZA RICONVALIDA MODELLO 
            ...
            try { DogManager.DogResult objResult = ErpContext.Instance.DogFactory.GetDog(dogId).Mnt<TipoRisorsa>(obj); }
