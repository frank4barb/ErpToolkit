...
namespace ErpToolkit.Controllers.SIO.HealthData
{
    public class TipoDatoClinicoController : ControllerErp
    {
        ...
        public IActionResult Edit([FromBody] ModelParam parms)  
        {
            ...
            {
                try { obj = ErpContext.Instance.DogFactory.GetDog(dogId).Row<TipoDatoClinico>(parms.Id); }
        ...
        public IActionResult Save([FromBody] ModelObject dataObj)
        {
            ...
            {
                ModelState.AddModelError(string.Empty, "Oggetto TipoDatoClinico non valido. null");
                return PartialView("~/Views/SIO/HealthData/TipoDatoClinico/_PartialEdit.cshtml", null);
            }
            TipoDatoClinico obj = System.Text.Json.JsonSerializer.Deserialize<TipoDatoClinico>((System.Text.Json.JsonElement)dataObj.data);
            ModelState.Clear(); //FORZA RICONVALIDA MODELLO 
            ...
            try { DogManager.DogResult objResult = ErpContext.Instance.DogFactory.GetDog(dogId).Mnt<TipoDatoClinico>(obj); }
