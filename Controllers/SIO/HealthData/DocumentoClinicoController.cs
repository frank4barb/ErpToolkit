...
namespace ErpToolkit.Controllers.SIO.HealthData
{
    public class DocumentoClinicoController : ControllerErp
    {
        ...
        public IActionResult Save([FromBody] ModelObject dataObj)
        {
            ...
            {
                ModelState.AddModelError(string.Empty, "Oggetto DocumentoClinico non valido. null");
                return PartialView("~/Views/SIO/HealthData/DocumentoClinico/_PartialEdit.cshtml", null);
            }
            DocumentoClinico obj = System.Text.Json.JsonSerializer.Deserialize<DocumentoClinico>((System.Text.Json.JsonElement)dataObj.data);
            ModelState.Clear(); //FORZA RICONVALIDA MODELLO 
            ...
            try { DogManager.DogResult objResult = ErpContext.Instance.DogFactory.GetDog(dogId).Mnt<DocumentoClinico>(obj); }
