...
namespace ErpToolkit.Controllers.SIO.Common
{
    public class TipoRichiestaController : ControllerErp
    {
        ...
        public IActionResult Save([FromBody] ModelObject dataObj)
        {
            ...
            {
                ModelState.AddModelError(string.Empty, "Oggetto TipoRichiesta non valido. null");
                return PartialView("~/Views/SIO/Common/TipoRichiesta/_PartialEdit.cshtml", null);
            }
            TipoRichiesta obj = System.Text.Json.JsonSerializer.Deserialize<TipoRichiesta>((System.Text.Json.JsonElement)dataObj.data);
            ModelState.Clear(); //FORZA RICONVALIDA MODELLO 
            ...
            try { DogManager.DogResult objResult = ErpContext.Instance.DogFactory.GetDog(dogId).Mnt<TipoRichiesta>(obj); }
