...
namespace ErpToolkit.Controllers.SIO.Act
{
    public class TipoEpisodioController : ControllerErp
    {
        ...
        public IActionResult Edit([FromBody] ModelParam parms)  
        {
            ...
            {
                try { obj = ErpContext.Instance.DogFactory.GetDog(dogId).Row<TipoEpisodio>(parms.Id); }
        ...
        public IActionResult Save([FromBody] ModelObject dataObj)
        {
            ...
            {
                ModelState.AddModelError(string.Empty, "Oggetto TipoEpisodio non valido. null");
                return PartialView("~/Views/SIO/Act/TipoEpisodio/_PartialEdit.cshtml", null);
            }
            TipoEpisodio obj = System.Text.Json.JsonSerializer.Deserialize<TipoEpisodio>((System.Text.Json.JsonElement)dataObj.data);
            ModelState.Clear(); //FORZA RICONVALIDA MODELLO 
            ...
            try { DogManager.DogResult objResult = ErpContext.Instance.DogFactory.GetDog(dogId).Mnt<TipoEpisodio>(obj); }
