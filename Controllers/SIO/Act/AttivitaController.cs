...
namespace ErpToolkit.Controllers.SIO.Act
{
    public class AttivitaController : ControllerErp
    {
        ...
        public IActionResult Save([FromBody] ModelObject dataObj)
        {
            ...
            {
                ModelState.AddModelError(string.Empty, "Oggetto Attivita non valido. null");
                return PartialView("~/Views/SIO/Act/Attivita/_PartialEdit.cshtml", null);
            }
            Attivita obj = System.Text.Json.JsonSerializer.Deserialize<Attivita>((System.Text.Json.JsonElement)dataObj.data);
            ModelState.Clear(); //FORZA RICONVALIDA MODELLO 
            ...
            try { DogManager.DogResult objResult = ErpContext.Instance.DogFactory.GetDog(dogId).Mnt<Attivita>(obj); }
